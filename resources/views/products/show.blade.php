            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 sm:p-10">
                ...
                <div class="flex flex-col justify-between">
                    <div>
                        ...
                        <!-- Price Badge -->
                        <div class="mb-6">
                            <p class="text-3xl font-bold text-blue-600">Rp {{ number_format($product->price, 2, ',', '.') }}</p>
                        </div>
                        ...
                        <!-- Description -->
                        <div>
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-2">Description</h3>
        ...
        <div class="mt-12">
            ...
            @if(auth()->check() && (auth()->user()->role == 'mahasiswa' || auth()->user()->role == 'admin'))
                <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-10 mb-8">
                    <h2 class="text-2xl font-bold text-gray-900 mb-6">Share Your Review</h2>
                    <form method="POST" action="{{ route('reviews.store') }}" class="space-y-4">
                        @csrf
                        <input type="hidden" name="product_id" value="{{ $product->id }}" />
                        
                        <div>
                            <label for="rating" class="block text-sm font-semibold text-gray-900 mb-3">Product Rating</label>
                            <div class="flex items-center space-x-2">
                                ...
                                <span id="ratingValue" class="ml-2 text-sm font-medium text-gray-600">Choose a rating</span>
                            </div>
                        </div>

                        <div>
                            <label for="comment" class="block text-sm font-semibold text-gray-900 mb-2">Comment (Optional)</label>
                            <textarea name="comment" id="comment" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" placeholder="Share your experience with this product..."></textarea>
                        </div>
                    ...
                    <script>
                        ...
                        ratingStars.forEach(star => {
                            star.addEventListener('click', function(e) {
                                ...
                                ratingValue.textContent = value + ' out of 5';
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-10">
                ...
                @empty
                    <div class="text-center py-12">
                        ...
                        <p class="mt-4 text-gray-600 text-lg">No reviews for this product yet</p>
                        <p class="text-gray-500 text-sm mt-1">Be the first to leave a review</p>
                    </div>
                @endforelse
            </div>
        </div>
